...
import json
import re
# from botocore.awsrequest import AWSRequest # これらはもう不要です
# from botocore.auth import SigV4Auth # これらはもう不要です
# import urllib.request # これらはもう不要です
# from urllib.error import URLError # これらはもう不要です

# グローバル変数としてクライアントを初期化（Lambdaの場合、関数の外で初期化するとコールドスタート時のオーバーヘッドを削減できます）
# Lambdaの実行ロールに適切な権限が付与されていることを確認してください。
...
# Claude 3 Haiku はオンデマンドをサポートするため、inferenceProfile は不要です
# INFERENCE_PROFILE_ARN = "..." # この行は削除またはコメントアウト
...
def get_response_with_retry_bedrock(system_text, send_text, mode, max_retries=5, initial_delay=1):
    prompt = f"""
    <system>
    {system_text}
    </system>
    ...
    while retries < max_retries:
        try:
        ...
        except bedrock_runtime_client.exceptions.ThrottlingException as e:
            retries += 1
            print(f"ThrottlingException発生（{retries}/{max_retries}回目）。{current_delay}秒待機してリトライします。")
            time.sleep(current_delay)
            current_delay *= 2 # 次の待機時間を倍にする（指数バックオフ）
            ...
            print("Bedrock APIへのアクセスが拒否されました。IAM権限を確認してください。")
            raise

        except bedrock_runtime_client.exceptions.ValidationException as e:
            print(f"Validation Error: {e}")
            print("リクエストパラメータ（モデルID、プロンプト形式など）を確認してください。")
            raise
    ...
    # ループを抜けたがresponse_textが空の場合（Throttling以外のエラーでraiseされた場合など）
    if not response_text:
        raise Exception("Bedrockからの応答が得られませんでした。")

    # --- タグ抽出処理 ---
    question = re.findall(r"<question>(.*?)</question>", response_text, re.DOTALL)
    if not question:
        raise ValueError("<question>タグが見つかりませんでした。")

    options = []
    if mode != "記述式問題":
        options_bundles = re.findall(r"<options>(.*?)</options>", response_text, re.DOTALL)
        if not options_bundles:
            raise ValueError("<options>タグが見つかりませんでした。")

        for options_bundle in options_bundles:
            options_found = re.findall(r"<li>(.*?)</li>", options_bundle, re.DOTALL)
            if not options_found:
                raise ValueError("<li>タグが見つかりませんでした（選択肢）。")
            options.extend(options_found)

    answer = []
    if mode == "４択複数選択問題":
        answer_bundles = re.findall(r"<answer>(.*?)</answer>", response_text, re.DOTALL)
        if not answer_bundles:
            raise ValueError("<answer>タグが見つかりませんでした。")

        for answer_bundle in answer_bundles:
            answer_found = re.findall(r"<li>(.*?)</li>", answer_bundle, re.DOTALL)
            if not answer_found:
                raise ValueError("<li>タグが見つかりませんでした（解答）。")
            answer.extend(answer_found)
    else:
        answer_single = re.findall(r"<answer>(.*?)</answer>", response_text, re.DOTALL)
